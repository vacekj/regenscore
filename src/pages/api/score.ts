...
async function getERC20Transactions(address: string) {
  ...
  */
  try {
    const response = await fetchRequest('https://api.etherscan.io/api?module=account&action=tokentx&address=' + address + apikey)
...
async function getNormalTransactions(address: string) {
  ...
  */
  try {
    const response = await fetchRequest('https://api.etherscan.io/api?module=account&action=txlist&address=' + address + apikey)
...
async function getERC721Transactions(address: string) {
  ...
  */
  try {
    const response = await fetchRequest('https://api.etherscan.io/api?module=account&action=tokennfttx&address=' + address + apikey)
...
async function getTokenBalance(contractAddress: string, address: string) {
  ...
  */
   try {
    const response = await fetchRequest('https://api.etherscan.io/api?module=account&action=tokenbalance&contractaddress='
...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  // @ts-ignore
  const score = createScore(body.address);
